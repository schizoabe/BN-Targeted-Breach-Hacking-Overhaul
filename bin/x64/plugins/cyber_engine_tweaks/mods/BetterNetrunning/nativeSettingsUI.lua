...
function NativeSettingsUI.Build(nativeSettings, SettingsManager, TweakDBSetup)
    ...
    nativeSettings.addSwitch("/BetterNetrunning/Breaching", "Mark Beams", "Draws a network beam from V to each marked entity. Beams appear on mark and disappear on unmark.", settings.MarkBeamsEnabled, true, function(state)
        ...
        SettingsManager.Save()
        local markingSystem = Game.GetScriptableSystemsContainer():Get("BetterNetrunning.Marking.MarkingStateSystem")
        if markingSystem then
            markingSystem:RefreshMarkBeams()
        end
    end)
